...
app.get('/', (req, res) => {
    res.send(`
        ...
        <ul>
            ...
            <li><b>POST /api/files/upload</b> - Upload a single file (field name: "file")</li>
        ...
        <p><strong>Important:</strong> Only JPG, PNG, and PDF files are allowed. Maximum file size is 10MB.</p>
    `);
});

// Handler for single file upload - /api/upload
...
// Handler for single file upload - /api/files/upload
app.post('/api/files/upload', upload.single('file'), (req, res) => {
    console.log('Upload request received at /api/files/upload');
    console.log('Headers:', req.headers);
    ...
    if (err instanceof multer.MulterError) {
        ...
        if (err.code === 'LIMIT_UNEXPECTED_FILE') {
            return res.status(400).json({ 
                success: false, 
                message: 'Unexpected field name. Use "file" for single uploads or "files" for multiple uploads.' 
            });
        }
